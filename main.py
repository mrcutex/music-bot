...
async def search_yt(query):
    try:
        ...
        if 'result' in result and result['result']:
            ...
            logger.debug(f"search_yt: title={title}, duration={duration}, link={link}")
...
async def generate_queue_image(queue, chat_title):
    ...
    title_font = ImageFont.truetype("arialbd.ttf", 70) if os.path.exists("arialbd.ttf") else ImageFont.load_default()
    track_font = ImageFont.truetype("arial.ttf", 42) if os.path.exists("arial.ttf") else ImageFont.load_default()
    meta_font = ImageFont.truetype("arial.ttf", 36) if os.path.exists("arial.ttf") else ImageFont.load_default()
    
    header_height = 220
    draw.rectangle((0, 0, WIDTH, header_height), fill=ACCENT_COLOR)
    draw.text((80, 80), f"✦ {chat_title[:27]}", fill=TEXT_COLOR, font=title_font)
    
    card_padding = 40
    draw.rounded_rectangle(
        (card_padding, header_height + 50, WIDTH - card_padding, HEIGHT - 100),
    ...
    for idx, track in enumerate(queue[:max_tracks], 1):
        ...
        if idx % 2 == 0:
            draw.rounded_rectangle(
                (card_padding + 20, track_card_top, WIDTH - card_padding - 20, track_card_bottom),
                radius=15, fill=BG_COLOR
            )
        
        draw.text((card_padding + 50, track_card_top + 45), f"{idx}.", fill=ACCENT_COLOR, font=track_font)
        title = textwrap.shorten(track['title'], width=32, placeholder="..")
        draw.text((card_padding + 150, track_card_top + 40), title, fill=TEXT_COLOR, font=track_font)
        draw.text((card_padding + 150, track_card_top + 85), f"⏱︎ {track['duration']} | {track['media_type'].upper()}", fill=META_COLOR, font=meta_font)
        ...
        for i in range(14):
            ...
            draw.rounded_rectangle(
                (x, track_card_top + 50, x + bar_width, track_card_top + 50 + height),
...
async def enhanced_skip(client, message: Message):
    ...
    try:
        ...
        actual_position = requested_index - 1
...
async def image_queue(client, message):
    ...
    try:
        ...
        caption = f"**Current Queue for {chat_title}**\nTotal {len(queue)} songs in queue"
        await message.reply_photo(photo=img_path, caption=caption, quote=True)
        os.remove(img_path)
    except Exception as e:
